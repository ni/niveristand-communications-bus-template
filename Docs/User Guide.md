...
- Add a new **Page** referencing the GUID to the [custom device XML](https://niveristand-custom-device-handbook.readthedocs.io/en/latest/Planning_The_Custom_Device.html#xml-declaration)
...
This template makes use of the VeriStand [ActionVIOnCompile](https://niveristand-custom-device-handbook.readthedocs.io/en/latest/Custom_Device_Tips_and_Tricks.html#using-action-vis) to construct the **Execution Unit(s)** and associated channels required for executing the engine code.
